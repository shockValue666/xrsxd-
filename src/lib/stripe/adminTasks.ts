...
export const copyBillingDetailsToCustomer = async (uuid:string,payment_method:Stripe.PaymentMethod) => {
    ...
    try {
        await db.update(users).set({billingAddress:{...address},paymentMethod:{...payment_method[payment_method.type]}}).where(eq(users.id,uuid))
    } catch (error) {
        throw new Error(`Error updating user billing details: ${error}`)
    }
...
export const manageSubscriptionStatusChange = async (
    ...
    try {
        ...
        console.log(`Subscription ${subscription.id} inserted/updated successfully`);
