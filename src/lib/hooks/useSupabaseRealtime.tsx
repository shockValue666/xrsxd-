    ...
    const channel = supabase
      ...
      .on("postgres_changes",{event:"*",schema:"public",table:"files"},async (payload)=>{
        ...
        if(payload.eventType==="INSERT"){
          ...
          if(!state.workspaces.find((workspace)=>workspace.id===workspaceId)
            ...
            )
            {
              ...
              dispatch({type:'ADD_FILE',payload:{workspaceId, folderId, file:newFile}})
            } 
          ...
          if(fileExists && workspaceId && folderId){
            ...
            dispatch({type:'DELETE_FILE',payload:{workspaceId,folderId,fileId:payload.old.id}})
          }
              ...
              folder.files.some(file=>
                {
                  if(file.id===payload.new.id){
                    dispatch({type:'UPDATE_FILE',payload:{workspaceId,folderId,fileId:payload.new.id,file:{title:payload.new.title,iconId:payload.new.icon_id,inTrash:payload.new.in_trash}}})
                  }
                }
              )
            )
